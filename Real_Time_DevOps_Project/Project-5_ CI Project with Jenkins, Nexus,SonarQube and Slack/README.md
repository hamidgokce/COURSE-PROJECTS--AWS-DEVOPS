...
* Jenkins Userdata script (https://www.jenkins.io/doc/book/installing/linux/)
...
- Go to browser, `http://<public_ip_of_jenkins_server>:8080`, enter initialAdminPassword. We will also install suggested plugins. Then we will create our first admin user.
...
- We need to create a private repository in the Github to be used with this project. ((images/Git_Code_Migration.pdf))
...
```sh
git clone git@github.com:hamidgokce/COURSE-PROJECTS--AWS-DEVOPS.git
```
...
- We will create a New Job(pipeline) in Jenkins with below properties:
```sh
Pipeline from SCM 
Git
